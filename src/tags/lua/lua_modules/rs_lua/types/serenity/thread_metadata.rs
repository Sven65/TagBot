	...
	fn add_methods<'lua, T: rlua::UserDataMethods<'lua, Self>>(methods: &mut T) {
		...
		methods.add_meta_method(MetaMethod::Index, |ctx, this, value: String| {
			Ok(match value.as_str() {
				"archived" => convert_type(this.0.archived, ctx)?,
				"auto_archive_duration" => convert_type_option(this.0.auto_archive_duration, ctx)?,
				"archive_timestamp" => convert_constructable_option::<TBTimestamp, _>(this.0.archive_timestamp, ctx)?,
				"locked" => convert_type(this.0.locked, ctx)?,
				"create_timestamp" => convert_constructable_option::<TBTimestamp, _>(this.0.create_timestamp, ctx)?,
				"invitable" => convert_type(this.0.invitable, ctx)?,
				_ => Value::Nil,
			})
		})
	}
