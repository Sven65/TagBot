...
pub async fn init_commands() {
	...
	COMMAND_INDEX.lock().await.register_command("raw", |data, ctx| Box::pin(raw(data, ctx)), Some("Sends the raw data of a tag"), Some(raw_options_creator), true, None).await;

	COMMAND_INDEX.lock().await.register_command("stealtag", |data, ctx| Box::pin(stealtag(data, ctx)), Some("Steals a tag"), Some(stealtag_options_creator), false, None).await;
	
	COMMAND_INDEX.lock().await.register_command("tagowner", |data, ctx| Box::pin(tagowner(data, ctx)), Some("Shows who owns a tag"), Some(tagowner_options_creator), false, None).await;
	
	COMMAND_INDEX.lock().await.register_command("tag", |data, ctx| Box::pin(tag(data, ctx)), Some("Executes a tag"), Some(tag_options_creator), false, None).await;
